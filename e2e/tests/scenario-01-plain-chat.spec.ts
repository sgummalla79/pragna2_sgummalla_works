...
test.describe('Scenario 1 — Plain chat', () => {
  ...
  test('user message + assistant reply round-trip', async ({ page }) => {
    ...
    // NOTE: the smooth-reveal layer (`chat-markdown--animate` + the
    // per-character cadence) is a transient, streaming-only presentation
    // state — asserting its visibility here is inherently racy (the class
    // is gone once the turn settles, and blocks are mid-fade). It's
    // covered deterministically by the `useSmoothStreamingText` unit tests
    // and visually by manual scenario M1, so we intentionally do NOT
    // assert it in this browser round-trip.
